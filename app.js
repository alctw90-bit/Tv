...
function renderApplicationView() {
    ...
    if (currentFilterCategory === 'favorites') {
        ...
        if(DOM.sectionTitle) DOM.sectionTitle.textContent = 'My Premium Bookmarks';
    } else if (currentFilterCategory !== 'all') {
        // এই লজিকটি Sports বা যেকোনো ক্যাটাগরিকে নিখুঁতভাবে ফিল্টার করে
    ...
    // UI একটিভ বাটন স্টেট সিঙ্ক করার জন্য (সব বাটনের Active ক্লাস হ্যান্ডেল করা)
...
function syncActiveButtonStates() {
    ...
    allFilterButtons.forEach(btn => {
        if (btn.dataset.category === currentFilterCategory) {
            ...
            if(btn.classList.contains('mobile-filter-pill') && btn.dataset.category !== 'Sports') {
...
function bootstrapStreamPlayback(channelId) {
    ...
    if(DOM.playingCountry) DOM.playingCountry.innerHTML = `<i class="fa-solid fa-earth-americas mr-1"></i> ${targetChannel.country}`;
    ...
    if (Hls.isSupported()) {
        ...
        hls.on(Hls.Events.MANIFEST_PARSED, () => {
            DOM.videoPlayer.play().catch(() => console.log('Autoplay blocked.'));
            if(DOM.playerLoader) DOM.playerLoader.classList.add('hidden');
        });
        hls.on(Hls.Events.ERROR, function (event, data) {
            if (data.fatal && DOM.playerLoader) {
...
function updatePlayerFavoriteButtonState(isActive) {
    ...
    if (icon) {
        icon.className = isActive ? 'fa-solid fa-heart text-rose-500' : 'fa-regular fa-heart text-slate-300';
    }
}
...
function pushStreamHistoryLog(id) {
    ...
    if (historyChannels.length > 6) historyChannels.pop();
...
function setupEventBindings() {
    ...
    if(DOM.searchBar) DOM.searchBar.addEventListener('input', performSearch);
    if(DOM.searchBarMobile) DOM.searchBarMobile.addEventListener('input', performSearch);
    ...
    // গ্লোবাল ক্লিক লিসেনার: এটি সাইডবার, মোবাইল বার এবং আপনার নতুন "🔥 Live Sports" রো-বাটন সবকিছুর ক্লিক নিখুঁতভাবে হ্যান্ডেল করবে
